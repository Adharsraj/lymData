import React, { useState } from 'react';

const FooterNew = () => {
    const [isHovered, setIsHovered] = useState(false);

  return (
    <>
      <div className="hidden md:block max-w-[1640px]  mx-auto xl:px-32 md:px-16  font-dm  pb-10   md:text-xl bg-[#131313] text-white ">
        <div className=" md:flex justify-between  border-b pb-10 pt-10">
          <div className="">
            <h3 className="mb-4">LymData</h3>
            <div className="md:flex gap-4 hidden ">
            <h4 className="pb-1">overview</h4>
              <h4 className="pb-1">Features</h4>
              <h4 className="pb-1">Pricing</h4>
              <h4 className="pb-1">Careers</h4>
              <h4 className="pb-1">Help</h4>
              <h4 className="pb-1">Pricing</h4>
            </div>
            <div className="md:hidden gap-4 flex justify-between ">
              <div>
              <h4 className="pb-1">overview</h4>
              <h4 className="pb-1">Features</h4>
              <h4 className="pb-1">Pricing</h4>
              </div>
              <div>
              <h4 className="pb-1">Careers</h4>
              <h4 className="pb-1">Help</h4>
              <h4 className="pb-1">Pricing</h4>
              </div>
            </div>
            <div className="lg:hidden md:pb-10 pt-10  w-[280px]">
              <label htmlFor="stay uptodate mb-5">Stay upto date</label>
              <div className="md:flex gap-3 mt-5">
                <input
                  type="text"
                  className="border rounded-sm md:rounded-full mb-2 p-2"
                />
                <button className="bg-gray-500 text-white rounded-sm md:rounded-full p-2">
                  subscribe
                </button>
              </div>
            </div>
          </div>
          <div className="hidden lg:block">
      <label htmlFor="stay uptodate">Stay upto date</label>
      <div className="flex mt-3">
        <div className="relative flex">
          <input type="text" className="border rounded-full p-1 px-8" placeholder="Enter your email" />
          <button
            className="bg-gray-500 w-28 text-white rounded-full p-1 ml-[-30px] relative overflow-hidden group"
            onMouseEnter={() => setIsHovered(true)}
            onMouseLeave={() => setIsHovered(false)}
          >
           {isHovered?"➜":"Subscribe"}
          </button>
        </div>
      </div>
    </div>



        </div>

        <div className=" hidden md:flex justify-between lg:pt-10">
          <div>
            <h3>2023 Lymdata.All rights reserved</h3>
          </div>
          <div className="flex gap-10">
          <h4 className="pb-1">Terms</h4>
              <h4 className="pb-1">Privacy</h4>
              <h4 className="pb-1">Cookies</h4>
          </div>
        </div>
        <div className="  md:hidden justify-between">
          <div className="flex gap-10">
          <h4 className="pb-1">Terms</h4>
              <h4 className="pb-1">Privacy</h4>
              <h4 className="pb-1">Cookies</h4>
          </div>
          <div>
            <h3 className="text mt-5">2023 Lymdata.All rights reserved</h3>
          </div>
        </div>
      </div>
      <div className="md:hidden bg-[#131313] text-white">
        <div className="">
          <h1 className="px-2 mt-5 mb-5 font-bold text-lg">untitled ui</h1>
          <div className="flex gap-28 mb-5 px-4">
            <div>
              <h4 className="pb-1">overview</h4>
              <h4 className="pb-1">Features</h4>
              <h4 className="pb-1">Pricing</h4>
            </div>
            <div>
              <h4 className="pb-1">Careers</h4>
              <h4 className="pb-1">Help</h4>
              <h4 className="pb-1">Pricing</h4>
            </div>
          </div>
          <label className="px-3 font-bold" htmlFor="sta ">Stay upto date</label>
              <div className=" gap-3 mt-5 px-3 mb-5 border-b pb-6">
                <input
                  type="text"
                  className="border rounded-full   md:rounded-full w-full mb-2 p-2"
                />
                <button className="bg-gray-500 text-white rounded-full md:rounded-full w-full p-2">
                  subscribe
                </button>
              </div>
              <div className=" justify-between px-3">
          <div className="flex gap-10">
            <h4>Terms</h4>
            <h4>Privacy</h4>
            <h4>Cookies</h4>
          </div>
          <div>
            <h3 className="text mt-5 pb-5">2023 Lymdata.All rights reserved</h3>
          </div>
        </div>
        </div>
      </div>
    </>
  );
};

export default FooterNew;
